...
static int responseGetDataCallProfile(Parcel &p, void *response, size_t responselen) {
    ...
    num = *((int *) response);
    if (num > (int)(responselen / sizeof(RIL_DataCallProfileInfo))) {
...
static void onNewCommandConnect() {
    ...
    RIL_onUnsolicitedResponse(RIL_UNSOL_RIL_CONNECTED_V4,
...
processRadioState(RIL_RadioState newRadioState) {
    ...
    if((newRadioState > RADIO_STATE_UNAVAILABLE) && (newRadioState < RADIO_STATE_ON)) {
        ...
        if(newVoiceRadioTech != voiceRadioTech) {
            ...
            RIL_onUnsolicitedResponse (RIL_UNSOL_VOICE_RADIO_TECH_CHANGED_V4,
                                ...
                                size_t datalen)
{
    ...
    for (int i = 0; i < (int)NUM_ELEMS(unsolResponse_conv); i++) {
        if (unsolResponse_conv[i].v4 == unsolResponse) {
            unsolResponse = unsolResponse_conv[i].curr;
            RLOGD("RIL_onUnsolicitedResponse %d -> %d",
                unsolResponse_conv[i].v4, unsolResponse);
